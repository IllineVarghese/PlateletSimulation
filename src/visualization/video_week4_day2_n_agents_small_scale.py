...
RESULTS_DIR = "results"
OUTPUT_MOVIE = os.path.join(RESULTS_DIR, "week4_day2_n_agents_small_scale.mp4")
OUTPUT_IMAGE = os.path.join(RESULTS_DIR, "week4_day2_n_agents_small_scale.png")
...
def make_flow_arrows():
    ...
    return pdata.glyph(orient="vectors", scale="mag", factor=0.35)
...
def make_agent_sphere(center):
    return pv.Sphere(radius=0.12, center=center, theta_resolution=32, phi_resolution=32)


def make_outline_sphere(center):
    return pv.Sphere(radius=0.135, center=center, theta_resolution=32, phi_resolution=32)
...
def add_static_scene(plotter):
    ...
    chemical_contours = chemical_field.contour(isosurfaces=[0.35, 0.65])

    plotter.add_mesh(vessel, color="lightsteelblue", opacity=0.12, smooth_shading=True)
    plotter.add_mesh(inlet_ring, color="navy", line_width=3)
    plotter.add_mesh(outlet_ring, color="navy", line_width=3)
    plotter.add_mesh(flow_arrows, color="lightsteelblue", opacity=0.18)
    ...
    plotter.add_mesh(chemical_contours, cmap="Reds", opacity=0.05, show_scalar_bar=False)
    ...
    plotter.add_text("Week 4 Day 2: Small N-Agent Visualization", position="upper_left", font_size=14)
...
def render_movie(agents, output_path: str, image_path: str) -> None:
    ...
    for i, agent in enumerate(agents):
        ...
        outline_actor = plotter.add_mesh(
            make_outline_sphere(start_center),
            color="dimgray",
            smooth_shading=True,
        )
...
def print_summary(agents) -> None:
    print("\n=== Week 4 Day 2: Small N-Agent Visualization ===")
    ...
    for agent in agents:
        ...
        print("Max shear:", max(agent.debug_history["shear_stress"]))
