...
RESULTS_DIR = "results"
OUTPUT_MOVIE = os.path.join(RESULTS_DIR, "week3_day4_two_agents_behavior.mp4")
OUTPUT_IMAGE = os.path.join(RESULTS_DIR, "week3_day4_two_agents_behavior.png")
...
def make_flow_arrows():
    ...
    return pdata.glyph(orient="vectors", scale="mag", factor=0.38)
...
def make_source_mesh():
    return pv.Sphere(radius=0.055, center=CHEMICAL_SOURCE_POSITION)
...
def make_agent_sphere(center, morphology_value: float):
    radius = 0.11 + 0.05 * morphology_value
    return pv.Sphere(radius=radius, center=center, theta_resolution=40, phi_resolution=40)


def make_outline_sphere(center, morphology_value: float):
    radius = 0.125 + 0.05 * morphology_value
    return pv.Sphere(radius=radius, center=center, theta_resolution=40, phi_resolution=40)


def make_halo_sphere(center, secretion_value: float):
    radius = 0.15 + 0.10 * secretion_value
    return pv.Sphere(radius=radius, center=center, theta_resolution=32, phi_resolution=32)
...
def make_trail_mesh(history_x, history_y, history_z):
    ...
    if len(points) < 2:
        return None
    return pv.Spline(points, len(points) * 6)
...
def add_static_scene(plotter):
    ...
    chemical_contours = chemical_field.contour(isosurfaces=[0.3, 0.6])

    plotter.add_mesh(vessel, color="lightsteelblue", opacity=0.16, smooth_shading=True)
    plotter.add_mesh(inlet_ring, color="navy", line_width=3)
    plotter.add_mesh(outlet_ring, color="navy", line_width=3)
    plotter.add_mesh(flow_arrows, color="lightsteelblue", opacity=0.28)
    plotter.add_mesh(centerline, color="black", line_width=2)

    plotter.add_mesh(chemical_field, scalars="concentration", cmap="Reds", opacity=0.03, show_scalar_bar=False)
    plotter.add_mesh(chemical_contours, cmap="Reds", opacity=0.08, show_scalar_bar=False)
    ...
    plotter.add_text("Week 3 Day 4: Two-Agent Behavioral Differentiation", position="upper_left", font_size=14)
    plotter.add_text("Agent A = blue", position=(0.02, 0.08), font_size=10, color="royalblue")
    plotter.add_text("Agent B = green", position=(0.02, 0.05), font_size=10, color="limegreen")
    plotter.add_text("Size = morphology, halo = secretion, trail width = stickiness", position=(0.02, 0.02), font_size=10, color="black")


def render_movie(agent_A: GRNAgent, agent_B: GRNAgent, output_path: str, image_path: str) -> None:
    ...
    start_A = (
        agent_A.debug_history["position_x"][0],
        agent_A.debug_history["position_y"][0],
        agent_A.debug_history["position_z"][0],
    )
    start_B = (
        agent_B.debug_history["position_x"][0],
        agent_B.debug_history["position_y"][0],
        agent_B.debug_history["position_z"][0],
    )

    start_morph_A = agent_A.debug_history["morphology"][0]
    start_morph_B = agent_B.debug_history["morphology"][0]
    start_sec_A = agent_A.debug_history["secretion_rate"][0]
    start_sec_B = agent_B.debug_history["secretion_rate"][0]

    halo_actor_A = plotter.add_mesh(make_halo_sphere(start_A, start_sec_A), color="royalblue", opacity=0.10, smooth_shading=True)
    halo_actor_B = plotter.add_mesh(make_halo_sphere(start_B, start_sec_B), color="limegreen", opacity=0.10, smooth_shading=True)

    outline_actor_A = plotter.add_mesh(make_outline_sphere(start_A, start_morph_A), color="dimgray", smooth_shading=True)
    outline_actor_B = plotter.add_mesh(make_outline_sphere(start_B, start_morph_B), color="dimgray", smooth_shading=True)

    actor_A = plotter.add_mesh(make_agent_sphere(start_A, start_morph_A), color="royalblue", smooth_shading=True)
    actor_B = plotter.add_mesh(make_agent_sphere(start_B, start_morph_B), color="limegreen", smooth_shading=True)
    ...
    n_steps = len(agent_A.debug_history["step"])
    snapshot_frame = n_steps // 2

    for i in range(n_steps):
        center_A = (
            agent_A.debug_history["position_x"][i],
            agent_A.debug_history["position_y"][i],
            agent_A.debug_history["position_z"][i],
        )
        center_B = (
            agent_B.debug_history["position_x"][i],
            agent_B.debug_history["position_y"][i],
            agent_B.debug_history["position_z"][i],
        )

        morph_A = agent_A.debug_history["morphology"][i]
        morph_B = agent_B.debug_history["morphology"][i]
        sec_A = agent_A.debug_history["secretion_rate"][i]
        sec_B = agent_B.debug_history["secretion_rate"][i]
        stick_A = agent_A.debug_history["stickiness"][i]
        stick_B = agent_B.debug_history["stickiness"][i]

        halo_actor_A.mapper.SetInputData(make_halo_sphere(center_A, sec_A))
        halo_actor_B.mapper.SetInputData(make_halo_sphere(center_B, sec_B))

        outline_actor_A.mapper.SetInputData(make_outline_sphere(center_A, morph_A))
        outline_actor_B.mapper.SetInputData(make_outline_sphere(center_B, morph_B))

        actor_A.mapper.SetInputData(make_agent_sphere(center_A, morph_A))
        actor_B.mapper.SetInputData(make_agent_sphere(center_B, morph_B))

        if trail_actor_A is not None:
            plotter.remove_actor(trail_actor_A)
        if trail_actor_B is not None:
            plotter.remove_actor(trail_actor_B)

        trail_A = make_trail_mesh(
            agent_A.debug_history["position_x"][: i + 1],
            agent_A.debug_history["position_y"][: i + 1],
            agent_A.debug_history["position_z"][: i + 1],
        )
        trail_B = make_trail_mesh(
            agent_B.debug_history["position_x"][: i + 1],
            agent_B.debug_history["position_y"][: i + 1],
            agent_B.debug_history["position_z"][: i + 1],
        )

        if trail_A is not None:
            trail_actor_A = plotter.add_mesh(trail_A, color="royalblue", line_width=3 + 6 * stick_A)
        if trail_B is not None:
            trail_actor_B = plotter.add_mesh(trail_B, color="limegreen", line_width=3 + 6 * stick_B)
...
def print_summary(agent_A: GRNAgent, agent_B: GRNAgent) -> None:
    ...
    print("Max morphology:", max(agent_A.debug_history["morphology"]))
    print("Max secretion:", max(agent_A.debug_history["secretion_rate"]))
    ...
    print("Max morphology:", max(agent_B.debug_history["morphology"]))
    print("Max secretion:", max(agent_B.debug_history["secretion_rate"]))
    print("Max stickiness:", max(agent_B.debug_history["stickiness"]))

    print("\nSaved movie:", OUTPUT_MOVIE)
    print("Saved image:", OUTPUT_IMAGE)


if __name__ == "__main__":
    agent_A, agent_B = run_simulation()
    render_movie(agent_A, agent_B, OUTPUT_MOVIE, OUTPUT_IMAGE)
