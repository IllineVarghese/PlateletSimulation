...
RESULTS_DIR = "results"
OUTPUT_MOVIE = os.path.join(RESULTS_DIR, "phase3_final_hero_visualization.mp4")
OUTPUT_IMAGE = os.path.join(RESULTS_DIR, "phase3_final_hero_visualization.png")
...
def create_agents(n_agents: int):
    agents = []
    y_positions = np.linspace(-0.55, 0.55, n_agents)
    z_positions = np.linspace(-0.35, 0.35, n_agents)

    for i in range(n_agents):
        name = f"A{i}"
        position = np.array([0.0, y_positions[i], z_positions[i],], dtype=float)

        # Keep points inside vessel radius
        radial = np.linalg.norm(position[1:3])
        if radial >= 0.9 * VESSEL_RADIUS:
            scale = (0.9 * VESSEL_RADIUS) / radial
            position[1] *= scale
            position[2] *= scale
...
def make_flow_arrows():
    xs = np.linspace(1.5, VESSEL_LENGTH - 1.5, 3)
    ys = np.linspace(-VESSEL_RADIUS * 0.5, VESSEL_RADIUS * 0.5, 3)
    zs = np.linspace(-VESSEL_RADIUS * 0.5, VESSEL_RADIUS * 0.5, 3)
    ...
    return pdata.glyph(orient="vectors", scale="mag", factor=0.28)
...
def make_agent_sphere(center, morphology_value: float, stickiness_value: float):
    radius = 0.07 + 0.05 * morphology_value
    sphere = pv.Sphere(radius=radius, center=center, theta_resolution=24, phi_resolution=24)
    sphere["stickiness"] = np.full(sphere.n_points, stickiness_value)
    return sphere


def make_outline_sphere(center, morphology_value: float):
    radius = 0.082 + 0.05 * morphology_value
    return pv.Sphere(radius=radius, center=center, theta_resolution=24, phi_resolution=24)
...
def add_static_scene(plotter):
    ...
    chemical_contours = chemical_field.contour(isosurfaces=[0.45, 0.70])

    plotter.add_mesh(vessel, color="lightsteelblue", opacity=0.08, smooth_shading=True)
    plotter.add_mesh(inlet_ring, color="navy", line_width=2)
    plotter.add_mesh(outlet_ring, color="navy", line_width=2)
    plotter.add_mesh(flow_arrows, color="lightsteelblue", opacity=0.10)
    plotter.add_mesh(centerline, color="black", line_width=1)

    plotter.add_mesh(chemical_field, scalars="concentration", cmap="Reds", opacity=0.01, show_scalar_bar=False)
    plotter.add_mesh(chemical_contours, cmap="Reds", opacity=0.03, show_scalar_bar=False)
    ...
    plotter.add_text("Phase 3 Final: N-Agent GRN Behavior in Vessel", position="upper_left", font_size=14)
    plotter.add_text("Color = Stickiness | Size = Morphology", position=(0.02, 0.03), font_size=10, color="black")
...
def render_movie(agents, output_path: str, image_path: str) -> None:
    ...
    for agent in agents:
        ...
        morph = agent.debug_history["morphology"][0]
        stick = agent.debug_history["stickiness"][0]

        outline_actor = plotter.add_mesh(
            make_outline_sphere(start_center, morph),
            color="dimgray",
            smooth_shading=True,
        )
        sphere_actor = plotter.add_mesh(
            make_agent_sphere(start_center, morph, stick),
            scalars="stickiness",
            clim=[0.0, 1.0],
            cmap="viridis",
            smooth_shading=True,
    ...
    for step_idx in range(n_steps):
        for i, agent in enumerate(agents):
            ...
            morph = agent.debug_history["morphology"][step_idx]
            stick = agent.debug_history["stickiness"][step_idx]

            outline_actors[i].mapper.SetInputData(make_outline_sphere(center, morph))
            sphere_actors[i].mapper.SetInputData(make_agent_sphere(center, morph, stick))
...
def print_summary(agents) -> None:
    print("\n=== Phase 3 Final Hero Visualization ===")
