...
RESULTS_DIR = "results"
OUTPUT_MOVIE = os.path.join(RESULTS_DIR, "week2_day3_single_agent_cylinder_flow_shear.mp4")
...
def compute_poiseuille_velocity_at_position(position: np.ndarray) -> float:
    """
    Axial velocity profile in cylindrical Poiseuille flow:
        u(r) = U_MAX * (1 - (r/R)^2)
    """
...
def compute_poiseuille_shear_from_gradient(position: np.ndarray) -> float:
    """
    Shear proxy from the magnitude of the velocity gradient:
        |du/dr| = 2 * U_MAX * r / R^2

    Normalize by the wall value:
        max = 2 * U_MAX / R
    so normalized shear becomes:
        r / R
    but here it is explicitly computed from the gradient.
    """
    r = compute_radial_distance(position)
...
def apply_agent_outputs(agent: GRNAgent, dt: float) -> None:
    """
    Day 3:
    Flow comes from Poiseuille field.
    Stickiness reduces effective transport speed.
    """
...
def render_movie(agent: GRNAgent, output_path: str) -> None:
    ...
    plotter.add_mesh(flow_arrows, color="lightsteelblue", opacity=0.55)
    ...
    plotter.add_text("Week 2 Day 3: Flow-Informed Shear Stress View", font_size=14)
    ...
    # Day 3: color platelet by shear stress
    ...
    for i in range(n_steps):
        ...
        new_sphere = make_agent_sphere(center, shear_value, scalar_name)
