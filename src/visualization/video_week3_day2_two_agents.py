...
RESULTS_DIR = "results"
OUTPUT_MOVIE = os.path.join(RESULTS_DIR, "week3_day2_two_agents.mp4")
...
def make_flow_arrows():
    xs = np.linspace(0.7, VESSEL_LENGTH - 0.7, 5)
...
def make_agent_mesh(center, color_name: str):
    return pv.Sphere(radius=0.14, center=center, theta_resolution=40, phi_resolution=40)
...
def render_movie(agent_A: GRNAgent, agent_B: GRNAgent, output_path: str) -> None:
    ...
    plotter.add_mesh(flow_arrows, color="lightsteelblue", opacity=0.40)
    plotter.add_mesh(centerline, color="black", line_width=2)

    plotter.add_mesh(chemical_field, scalars="concentration", cmap="Reds", opacity=0.04, show_scalar_bar=False)
    plotter.add_mesh(chemical_contours, cmap="Reds", opacity=0.10, show_scalar_bar=False)
    ...
    plotter.add_text("Week 3 Day 2: Two Independent Agents in Vessel", font_size=14)
    ...
    outline_A = pv.Sphere(radius=0.155, center=start_A, theta_resolution=40, phi_resolution=40)
    outline_B = pv.Sphere(radius=0.155, center=start_B, theta_resolution=40, phi_resolution=40)

    outline_actor_A = plotter.add_mesh(outline_A, color="dimgray", smooth_shading=True)
    outline_actor_B = plotter.add_mesh(outline_B, color="dimgray", smooth_shading=True)

    mesh_A = make_agent_mesh(start_A, "blue")
    mesh_B = make_agent_mesh(start_B, "green")

    actor_A = plotter.add_mesh(mesh_A, color="royalblue", smooth_shading=True)
    actor_B = plotter.add_mesh(mesh_B, color="limegreen", smooth_shading=True)
    ...
    for i in range(n_steps):
        ...
        new_outline_A = pv.Sphere(radius=0.155, center=center_A, theta_resolution=40, phi_resolution=40)
        new_outline_B = pv.Sphere(radius=0.155, center=center_B, theta_resolution=40, phi_resolution=40)
        outline_actor_A.mapper.SetInputData(new_outline_A)
        outline_actor_B.mapper.SetInputData(new_outline_B)

        new_A = make_agent_mesh(center_A, "blue")
        new_B = make_agent_mesh(center_B, "green")
        actor_A.mapper.SetInputData(new_A)
        actor_B.mapper.SetInputData(new_B)
        ...
        if trail_A is not None:
            trail_actor_A = plotter.add_mesh(trail_A, color="royalblue", line_width=6)
        if trail_B is not None:
            trail_actor_B = plotter.add_mesh(trail_B, color="limegreen", line_width=6)
...
if __name__ == "__main__":
    ...
    render_movie(agent_A, agent_B, OUTPUT_MOVIE)
