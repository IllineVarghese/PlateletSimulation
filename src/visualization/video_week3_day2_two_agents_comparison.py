...
RESULTS_DIR = "results"
OUTPUT_MOVIE = os.path.join(RESULTS_DIR, "week3_day2_two_agents_comparison.mp4")
OUTPUT_IMAGE = os.path.join(RESULTS_DIR, "week3_day2_two_agents_comparison.png")

DT = 0.1
STEPS = 100
FRAME_REPEAT = 3   # makes the video longer and easier to watch
...
def make_agent_sphere(center):
    return pv.Sphere(radius=0.13, center=center, theta_resolution=40, phi_resolution=40)


def make_outline_sphere(center):
    return pv.Sphere(radius=0.145, center=center, theta_resolution=40, phi_resolution=40)
...
def add_static_scene(plotter):
    ...
    plotter.add_mesh(flow_arrows, color="lightsteelblue", opacity=0.32)
    ...
    plotter.add_text("Week 3 Day 2: Two Independent Agents Comparison", position="upper_left", font_size=14)
    plotter.add_text("Agent A = blue (centerline / high flow)", position=(0.02, 0.08), font_size=10, color="royalblue")
    plotter.add_text("Agent B = green (near wall / high shear)", position=(0.02, 0.04), font_size=10, color="limegreen")
...
def render_movie(agent_A: GRNAgent, agent_B: GRNAgent, output_path: str, image_path: str) -> None:
    ...
    outline_actor_A = plotter.add_mesh(make_outline_sphere(start_A), color="dimgray", smooth_shading=True)
    outline_actor_B = plotter.add_mesh(make_outline_sphere(start_B), color="dimgray", smooth_shading=True)

    actor_A = plotter.add_mesh(make_agent_sphere(start_A), color="royalblue", smooth_shading=True)
    actor_B = plotter.add_mesh(make_agent_sphere(start_B), color="limegreen", smooth_shading=True)

    trail_actor_A = None
    trail_actor_B = None

    # Better fixed camera for side-by-side comparison
    plotter.camera_position = [
        (5.0, -5.6, 2.8),   # camera position
        (5.0, 0.0, 0.0),    # focal point
        (0.0, 0.0, 1.0),    # view up
    ]
    ...
    for i in range(n_steps):
        ...
        outline_actor_A.mapper.SetInputData(make_outline_sphere(center_A))
        outline_actor_B.mapper.SetInputData(make_outline_sphere(center_B))

        actor_A.mapper.SetInputData(make_agent_sphere(center_A))
        actor_B.mapper.SetInputData(make_agent_sphere(center_B))
        ...
        if trail_A is not None:
            trail_actor_A = plotter.add_mesh(trail_A, color="royalblue", line_width=7)
        if trail_B is not None:
            trail_actor_B = plotter.add_mesh(trail_B, color="limegreen", line_width=7)
