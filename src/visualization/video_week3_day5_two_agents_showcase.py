...
RESULTS_DIR = "results"
OUTPUT_MOVIE = os.path.join(RESULTS_DIR, "week3_day5_two_agents_showcase.mp4")
OUTPUT_IMAGE = os.path.join(RESULTS_DIR, "week3_day5_two_agents_showcase.png")
...
def add_static_scene(plotter):
    ...
    plotter.add_mesh(vessel, color="lightsteelblue", opacity=0.14, smooth_shading=True)
    plotter.add_mesh(inlet_ring, color="navy", line_width=3)
    plotter.add_mesh(outlet_ring, color="navy", line_width=3)
    plotter.add_mesh(flow_arrows, color="lightsteelblue", opacity=0.22)
    plotter.add_mesh(centerline, color="black", line_width=2)

    plotter.add_mesh(chemical_field, scalars="concentration", cmap="Reds", opacity=0.02, show_scalar_bar=False)
    plotter.add_mesh(chemical_contours, cmap="Reds", opacity=0.06, show_scalar_bar=False)
    ...
    plotter.add_text("Week 3 Day 5: Two-Agent Showcase", position="upper_left", font_size=14)
    plotter.add_text("Blue = Agent A", position=(0.02, 0.08), font_size=10, color="royalblue")
    plotter.add_text("Green = Agent B", position=(0.02, 0.05), font_size=10, color="limegreen")
    plotter.add_text("Size→morphology  Halo→secretion  Trail→stickiness", position=(0.02, 0.02), font_size=10, color="black")
...
def render_movie(agent_A: GRNAgent, agent_B: GRNAgent, output_path: str, image_path: str) -> None:
    ...
    morph_A = agent_A.debug_history["morphology"][0]
    morph_B = agent_B.debug_history["morphology"][0]
    sec_A = agent_A.debug_history["secretion_rate"][0]
    sec_B = agent_B.debug_history["secretion_rate"][0]

    halo_actor_A = plotter.add_mesh(make_halo_sphere(start_A, sec_A), color="royalblue", opacity=0.08, smooth_shading=True)
    halo_actor_B = plotter.add_mesh(make_halo_sphere(start_B, sec_B), color="limegreen", opacity=0.08, smooth_shading=True)

    outline_actor_A = plotter.add_mesh(make_outline_sphere(start_A, morph_A), color="dimgray", smooth_shading=True)
    outline_actor_B = plotter.add_mesh(make_outline_sphere(start_B, morph_B), color="dimgray", smooth_shading=True)

    actor_A = plotter.add_mesh(make_agent_sphere(start_A, morph_A), color="royalblue", smooth_shading=True)
    actor_B = plotter.add_mesh(make_agent_sphere(start_B, morph_B), color="limegreen", smooth_shading=True)
    ...
    for i in range(n_steps):
        ...
        if trail_A is not None:
            trail_actor_A = plotter.add_mesh(trail_A, color="royalblue", line_width=3 + 5 * stick_A)
        if trail_B is not None:
            trail_actor_B = plotter.add_mesh(trail_B, color="limegreen", line_width=3 + 5 * stick_B)
