...
RESULTS_DIR = "results"
SUMMARY_CSV = os.path.join(RESULTS_DIR, "week4_day4_n_agents_summary.csv")
SUMMARY_PLOT = os.path.join(RESULTS_DIR, "week4_day4_n_agents_summary.png")
...
def build_summary_rows(agents):
    ...
    for agent in agents:
        row = {
            ...
            "initial_y": agent.debug_history["position_y"][0],
            "final_x_position": agent.debug_history["position_x"][-1],
            ...
            "max_morphology": max(agent.debug_history["morphology"]),
            "max_secretion_rate": max(agent.debug_history["secretion_rate"]),
            "max_chemical_concentration": max(agent.debug_history["chemical_concentration"]),
        }
        rows.append(row)
...
def save_summary_csv(rows, output_path: str) -> None:
...
def save_summary_plot(rows, output_path: str) -> None:
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    indices = [row["agent_index"] for row in rows]
    final_x = [row["final_x_position"] for row in rows]
    final_speed = [row["final_speed"] for row in rows]
    max_shear = [row["max_shear"] for row in rows]
    max_stickiness = [row["max_stickiness"] for row in rows]

    fig, axes = plt.subplots(2, 2, figsize=(12, 8))

    axes[0, 0].plot(indices, final_x, marker="o")
    axes[0, 0].set_title("Final X Position vs Agent Index")
    axes[0, 0].set_xlabel("Agent Index")
    axes[0, 0].set_ylabel("Final X Position")

    axes[0, 1].plot(indices, final_speed, marker="o")
    axes[0, 1].set_title("Final Speed vs Agent Index")
    axes[0, 1].set_xlabel("Agent Index")
    axes[0, 1].set_ylabel("Final Speed")

    axes[1, 0].plot(indices, max_shear, marker="o")
    axes[1, 0].set_title("Max Shear vs Agent Index")
    axes[1, 0].set_xlabel("Agent Index")
    axes[1, 0].set_ylabel("Max Shear")

    axes[1, 1].plot(indices, max_stickiness, marker="o")
    axes[1, 1].set_title("Max Stickiness vs Agent Index")
    axes[1, 1].set_xlabel("Agent Index")
    axes[1, 1].set_ylabel("Max Stickiness")

    fig.suptitle("Week 4 Day 4: Larger N-Agent Scaling Summary")
    fig.tight_layout()
...
def print_summary(rows) -> None:
    print("\n=== Week 4 Day 4: Larger N-Agent Run ===")
    print("Total agents:", len(rows))

    final_speeds = [row["final_speed"] for row in rows]
    final_x_positions = [row["final_x_position"] for row in rows]
    max_shear_values = [row["max_shear"] for row in rows]
    max_stickiness_values = [row["max_stickiness"] for row in rows]

    print("Min final speed:", min(final_speeds))
    print("Max final speed:", max(final_speeds))
    print("Min final x:", min(final_x_positions))
    print("Max final x:", max(final_x_positions))
    print("Min max shear:", min(max_shear_values))
    print("Max max shear:", max(max_shear_values))
    print("Unique max stickiness values:", len(set(round(v, 4) for v in max_stickiness_values)))

    print("\nSaved summary CSV:", SUMMARY_CSV)
    print("Saved summary plot:", SUMMARY_PLOT)

    if len(set(round(v, 4) for v in final_speeds)) > 1:
        print("PASS: Larger N-agent run shows differentiated behavior.")
    else:
        print("WARNING: Larger N-agent run looks too uniform.")


if __name__ == "__main__":
    agents = run_simulation(N_AGENTS)
    rows = build_summary_rows(agents)
