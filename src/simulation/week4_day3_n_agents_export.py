...
RESULTS_DIR = "results"
SUMMARY_CSV = os.path.join(RESULTS_DIR, "week4_day3_n_agents_summary.csv")
SUMMARY_PLOT = os.path.join(RESULTS_DIR, "week4_day3_n_agents_summary.png")
...
def save_summary_plot(rows, output_path: str) -> None:
    ...
    fig.suptitle("Week 4 Day 3: Small N-Agent Scaling Summary")
    fig.tight_layout()
...
def print_summary(rows) -> None:
    print("\n=== Week 4 Day 3: Small N-Agent Export ===")
    print("Total agents:", len(rows))

    for row in rows:
        print(f"\n--- {row['agent_name']} ---")
        print("Initial y:", row["initial_y"])
        print("Final x position:", row["final_x_position"])
        print("Final speed:", row["final_speed"])
        print("Max shear:", row["max_shear"])
        print("Max stickiness:", row["max_stickiness"])
